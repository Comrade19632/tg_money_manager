...
class Form(StatesGroup):
    add_outcome_category_title = State()


@dp.message_handler(Command("add_outcome_category"))
@dp.message_handler(text="Добавить категорию расходов")
async def add_outcome_category(message: types.Message, state: FSMContext):
    await message.reply(
        "Укажите название категории",
    )
    await Form.add_outcome_category_title.set()


@dp.message_handler(state=Form.add_outcome_category_title)
