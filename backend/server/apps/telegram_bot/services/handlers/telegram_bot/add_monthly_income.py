...
class Form(StatesGroup):
    add_monthly_income_amount = State()
    add_monthly_income_date = State()
    add_monthly_income_category = State()
    add_monthly_income_title = State()


@dp.message_handler(Command("add_monthly_income"))
@dp.message_handler(text="Добавить месячный доход")
async def add_monthly_income(message: types.Message, state: FSMContext):
    await message.reply(
        "Укажите сумму месячного дохода",
    )
    await Form.add_monthly_income_amount.set()


@dp.message_handler(state=Form.add_monthly_income_amount)
async def process_income_amount(message: types.Message, state: FSMContext):
    ...
    await Form.add_monthly_income_date.set()


@dp.message_handler(state=Form.add_monthly_income_date)
async def process_date(message: types.Message, state: FSMContext):
    ...
    await Form.add_monthly_income_category.set()


@dp.message_handler(state=Form.add_monthly_income_category)
async def process_category(message: types.Message, state: FSMContext):
    ...
    await Form.add_monthly_income_title.set()


@dp.message_handler(state=Form.add_monthly_income_title)
